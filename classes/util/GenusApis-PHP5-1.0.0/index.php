...
switch($action) {
	...
	case 'authorize':
		...
		$oRequestToken = $oGenusApis->retrieveRequesttoken(array("friends.get", "users.get", "media.getAlbums"));
	...
	case 'authorized':
		...
		echo "<a href=\"".SCRIPT_URL."?action=mediaalbums&local_token=".$local_token."\">media.getAlbums</a><br />";
		break;
	case 'friends':
	...
	case 'mediaalbums':
		...
		$local_token = $_REQUEST['local_token'];
		$oAccessToken = getAccessTokenFromSession($local_token);
		$oXml = $oGenusApis->doMethod("media.getAlbums", array("userid" => $oAccessToken->getUserid()), $oAccessToken);
		echo "<pre>";
		print_r($oXml);
		echo "</pre>";
		break;
...
{
	echo "HyvesApiException Exception occured:<br>Code: ".$e->getCode()."<br>Message: ".$e->getMessage();
}
